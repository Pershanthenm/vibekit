...
export async function status({ root, args, force = false }) {
  ...
  await refreshRoadmap(root, project);
  const saved = await recordStatusChange(project, updated);
  const notes = [saved.memory && 'saved to agentmemory', saved.knowledge && 'published to OpenContext'].filter(Boolean);
