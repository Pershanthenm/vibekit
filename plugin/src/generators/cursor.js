...
function languageRuleFiles(project) {
  return languageRules(project).map(({ name, globs, rules }) =>
    rule(`vibecheck-${slugify(name)}`, { description: `${name} conventions`, globs }, bullets(rules)),
  );
}
...
const KNOWLEDGE_GATE = '- Before designing, check the OpenContext library (/opencontext-search or `vibecheck context <topic>`); record cross-project lessons with /opencontext-iterate.';
...
export function cursorFiles(project) {
  ...
  return [
    rule('vibecheck-workflow', { description: 'Spec-driven workflow gate', alwaysApply: true }, gate),
    rule(
    ...
    rule('vibecheck-specs', { description: 'How to write specs, plans and tasks', globs: 'specs/**' }, SPEC_WRITING),
    rule('vibecheck-tests', { description: 'Testing conventions', globs: TEST_GLOBS }, testingRule(project)),
    project.security.controls.length && rule('vibecheck-security', { description: 'Security baseline for auth, config and data access code', globs: SECURITY_GLOBS }, `${bullets(securityRules(project))}\n\nFull baseline: \`specs/security.md\`.`),
    project.docs.enabled && rule('vibecheck-docs', { description: 'Living documentation and diagrams', globs: `${project.docs.dir}/**` }, DOCS_RULE),
