...
import { EXAMPLE, TOOL_FREE_PATH, approveReview, exitCodeOf, fillSpec, newProject, patchProject, read, restoreEnv, runHook, sh, writeFileIn, writeTracedTests } from './helpers.js';
...
test('init seeds living docs for the project and a generated roadmap', async () => {
  ...
  assert.match(await read(root, '.cursor/rules/vibekit-docs.mdc'), /globs: docs\/\*\*/);
  await run(['feature', '--dir', root, 'Shared list']);
...
test('invalid Mermaid is rejected, and hooks protect the roadmap but allow doc edits', async () => {
  ...
  assert.match((await edit('docs/roadmap.md')).stderr, /generated by VibeKit/);
