...
```
vibekit new spec --answer what="…" --answer does="…; …" --answer never="…; …" --answer limits="…; …" --answer first="first: …; later: …"
```

It writes docs/spec.md in five numbered sections, one requirement sentence per pick, ingests it as BRS-001, and lists what is still thin. Read that back in two sentences: what was written, and what the analyst will ask about first. Then offer: "Start the analyst's questions" (`/vibekit:run-sprint`, then `/vibekit:answer`), "Let me edit docs/spec.md first" (then `vibekit ingest docs/spec.md`), or "Stop here".
