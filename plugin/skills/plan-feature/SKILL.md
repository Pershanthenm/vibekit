...
4. Fill the plan's `## Documentation` section: which docs this feature creates or changes (its feature doc, a design doc when there is UI, and architecture, data model or deployment diagrams if it touches them). Changing architecture means an ADR plus `vibekit playbook rearchitect`, not a quiet edit.
