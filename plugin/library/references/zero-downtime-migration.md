# Reference: zero-downtime-migration

Full source of the shipped skill `zero-downtime-migration`, kept for depth; the skill itself is the short version in `skills/lib/vibekit/zero-downtime-migration.md`.
Source: alirezarezvani/claude-skills · `engineering/skills/migration-architect/SKILL.md` @19392f7 · MIT. Copyright (c) 2025 Alireza Rezvani. Identity phrasing was removed; nothing else was edited.
The source bundled scripts; VibeKit ships none of them (a skill is data). Where the text says to run one, do by hand what the script checked.
