...
name: vpe-advisor
domain: c-level
triggers: [vpe advisor, thresholds, discipline on-call, deployment cadence, postmortem culture, sprint velocity dropping]
description: VP of Engineering advisory for startups: delivery throughput (DORA 4 metrics + bottleneck identification), engineering hiring funnel (sourcing → screen → onsite → offer conversion + time-to-fill + pipeline gap), engineering team structure (squad/tribe/chapter…
source: alirezarezvani/claude-skills · c-level-advisor/skills/vpe-advisor/SKILL.md @19392f7 · MIT
---
