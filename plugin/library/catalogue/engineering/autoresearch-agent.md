...
domain: engineering
triggers: [autoresearch agent, file measurable metric, inspired karpathys autoresearch, reset, loops indefinitely]
description: Autonomous experiment loop that optimizes any file by a measurable metric. Inspired by Karpathy's autoresearch. The agent edits a target file, runs a fixed evaluation, keeps improvements (git commit), discards failures (git reset), and loops indefinitely.
source: alirezarezvani/claude-skills · engineering/autoresearch-agent/skills/autoresearch-agent/SKILL.md @19392f7 · MIT
---
