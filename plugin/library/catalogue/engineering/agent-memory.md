...
name: agent-memory
domain: engineering
triggers: [agent memory, a projects claude, projects, memory line came, without human adopting]
description: Use when a project's CLAUDE.md has grown past what anyone reads and you want the agent to learn durable facts from its own sessions instead — or when asking why the agent keeps re-learning the same correction, why a remembered rule is wrong, or where a memory…
source: alirezarezvani/claude-skills · engineering/agent-memory/skills/agent-memory/SKILL.md @19392f7 · MIT
---
