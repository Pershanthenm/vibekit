...
name: agent-harness
domain: engineering
triggers: [agent harness, s domains, loop marketing work, finance domain self-verifying, want, through engineering harness]
description: Turn any domain folder of skills into a bounded agentic loop: compile a goal into a verifiable task plan, execute tasks with the domain's own tools, verify every task with machine-run checks, retry with caps, escalate to a human when budgets exhaust, and…
source: alirezarezvani/claude-skills · engineering/agent-harness/skills/agent-harness/SKILL.md @19392f7 · MIT
---
