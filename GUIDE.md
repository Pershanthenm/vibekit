...
**The skills that ship.** Every new project gets VibeKit's library of 27 engineering skills under `skills/lib/vibekit/`: short techniques, each with triggers and a test, indexed so an agent loads at most three per task. They are generated files, so an agent cannot edit them and `vibekit init` keeps them current, and they sit at the bottom of the override chain: `vibekit skills adopt tdd` copies one into `skills/lib/` as the team's own, which then wins; `vibekit skills reference tdd` prints the full source it was distilled from; `vibekit skills --for "add an index to orders"` shows which would load. A project that wants none of it: `vibekit init --no-library`, or `"library": false` in `specs/project.json`.

Behind the 27 sits the catalogue: 374 more skills from the same source, in 16 domains, shipped as data and indexed only when enabled. `vibekit skills catalogue` shows the domains with their counts; `vibekit skills catalogue seo --domain marketing` searches; `vibekit skills enable finance threat-detection` enables a domain and a skill, writes them into `skills/lib/vibekit/` with a short lead as the body and the full text behind `vibekit skills reference <name>`, and says what the index now costs. Every indexed skill is about forty always-loaded tokens, so enabling a whole large domain is refused past the budget cap unless you `--force` it. `vibekit skills disable <name|domain>` reverses it; the choice lives in `specs/project.json` under `catalogue`.
