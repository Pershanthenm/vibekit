# Setting up Vibe-check-cli

**Joining a team?** Follow [ONBOARDING.md](ONBOARDING.md): Windows, Mac and Linux, from a fresh machine.

Everything runs inside **Cursor**, with the **Claude Code extension** as your lead: a short bootstrap in Cursor's built-in terminal, then `/vibe-check-cli:setup` from the Claude panel does the rest. Your Multica board opens as a Cursor tab.
...
Both end in the same place: `/vibe-check-cli:health live` all green and `vibecheck multica selftest` passing.
